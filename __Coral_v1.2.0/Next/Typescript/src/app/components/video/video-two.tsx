...
export default function VideoTwo({className}:{className:string}) {
    let [open, setOpen] = useState(false)
  return (
    <>
        <Link href="#!" scroll={false} onClick={() =>setOpen(!open)} className="btn btn-icon btn-pills btn-primary m-1 lightbox"><FiVideo className="icons"/></Link><span className={`fw-bold text-uppercase small align-middle ms-2 ${className ? className : ''}`}>Watch Now</span>
