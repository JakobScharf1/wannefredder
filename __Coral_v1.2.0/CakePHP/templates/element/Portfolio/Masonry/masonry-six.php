...
<?php foreach ($portfolios as $item): ?>
    <div class="col-lg-2 col-md-4 picture-item">
        <div class="portfolio masonry rounded shadow position-relative overflow-hidden">
            ...
            <div class="content">
                <div class="title-body p-3 position-absolute start-0 end-0 m-4">
                    <a href="<?= $this->Url->build('/portfolio-detail-two/' . strtolower(str_replace(' ', '-', $item['title']))) ?>" class="text-white fs-5 fw-medium"><?= h($item['title']) ?></a>
                    <p class="text-white-50 mb-0"><?= h($item['name']) ?></p>
                </div>

                <div class="p-3 position-absolute top-0 end-0 m-4">
                    <a href="<?= $this->Url->build('/' . h($item['img'])) ?>" class="btn btn-sm btn-icon btn-pills btn-primary lightbox"><i data-feather="camera" class="fea icon-sm"></i></a>
                </div>
            </div>
        </div>
    </div>
