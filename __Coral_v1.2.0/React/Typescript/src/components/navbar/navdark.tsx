...
export default function Navdark({bg, tagline, position}:{bg:string, tagline:string, position:string}) {
    let [manu, setManu] = useState<string>('');
    let [submenu, setSubManu] = useState<string>('');
    let [toggle, setToggle] = useState<boolean>(false);
    let [scrolling, setScrolling] = useState<boolean>(false);
    let current:string = window.location.pathname
  ...
  return (
    <>
        <header id="topnav" className={`defaultscroll sticky ${scrolling ? 'nav-sticky' : ''} ${bg} ${tagline}`}>
            <div className="container">
                ...
                <ul className="buy-button list-inline mb-0">
                    ...
                    <li className="list-inline-item ps-1 mb-0">
                        <Link to="#" className="btn btn-sm btn-primary">Join Now</Link>
                    </li>
                </ul>
        
                <div id="navigation" style={{display : toggle ? 'block' : 'none'}}>
                    <ul className={`navigation-menu ${position}`}>
        ...
        <div className="offcanvas offcanvas-end shadow border-0" tabIndex={-1} id="offcanvasRight" aria-labelledby="offcanvasRightLabel">
            ...
            <div className="offcanvas-footer p-4 border-top text-center">
                <ul className="list-unstyled social-icon mb-0">
                    {footerSocial.map((item:SocialData,index:number)=>{
                        let Icon = item.icon
                        return(
