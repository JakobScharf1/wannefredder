        ...
        {blogData.slice(0,3).map((item,index)=>( 
            <div className="col-lg-4 col-md-6 col-12" key={index}>
                <div className="blog position-relative overflow-hidden shadow rounded p-4">
                    ...
                    <div className="pt-4">
                        <Link to={`/blog-standard-post/${item.id}`} className="text-dark title h5">{item.title}</Link>
                        <p className="text-muted my-2">{item.desc}</p>
                        <Link to={`/blog-standard-post/${item.id}`} className="text-dark read-more">Read More <i className="mdi mdi-chevron-right align-middle"></i></Link>
                    </div>
                </div>
            </div>
        ))}
