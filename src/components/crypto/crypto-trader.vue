<template>
    <div class="row justify-content-center g-4">
        <div v-for="(item, index) in data" :key="index" class="col-lg-3 col-md-6">
            <div class="position-relative p-4 bg-white shadow rounded overflow-hidden">
                <div class="pe-4">
                    <router-link to="#" class="link-title h5 text-dark">{{item.title}}</router-link>
                    <p class="text-muted mb-3 mt-4">{{item.desc}}</p>
                    <router-link to="#" class="btn btn-link primary">{{ item.btn }} <i class="mdi mdi-arrow-right align-middle"></i></router-link>
                </div>
                <div class="position-absolute top-50 start-100 translate-middle z-0">
                    <img :src="item.image" class="avatar avatar-large opacity-25" alt="">
                </div>
            </div>
        </div>
    </div>
